export default function ProfessorTable({ documentos, onValidate }) {
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full border border-gray-300 rounded-lg shadow">
        <thead className="bg-gray-100">
          <tr>
            <th className="px-4 py-2 border">Aluno</th>
            <th className="px-4 py-2 border">Categoria</th>
            <th className="px-4 py-2 border">Horas</th>
            <th className="px-4 py-2 border">Arquivo</th>
            <th className="px-4 py-2 border">Ações</th>
          </tr>
        </thead>
        <tbody>
          {documentos.map((doc) => (
            <tr key={doc.id} className="hover:bg-gray-50">
              <td className="px-4 py-2 border">{doc.aluno}</td>
              <td className="px-4 py-2 border">{doc.categoria}</td>
              <td className="px-4 py-2 border">{doc.horas}</td>
              <td className="px-4 py-2 border">
                <a
                  href={doc.arquivoUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 underline"
                >
                  Ver documento
                </a>
              </td>
              <td className="px-4 py-2 border space-x-2">
                <button
                  onClick={() => onValidate(doc.id, "aprovado")}
                  className="bg-green-500 text-white px-3 py-1 rounded"
                >
                  Aprovar
                </button>
                <button
                  onClick={() => onValidate(doc.id, "reprovado")}
                  className="bg-red-500 text-white px-3 py-1 rounded"
                >
                  Reprovar
                </button>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
}
